...
The Spring sample app has a few deployment files that we will use. The deployment file is a configuration for how OpenShift should run the app and defines the app's OpenShift objects to be created. 
...
Another yaml file is located at [`manifest/pullsecret.yaml`](https://github.ford.com/JPOTTE46/samples/blob/master/springboot/manifest/pullsecret.yaml). This file contains the Kubernetes secret to access the image that we will push to Quay. It will be referenced in each of the deployment files. 
...
Continue to [Manifest Definition](./8-manifest.md).
